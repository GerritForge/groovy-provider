...
class RedirectAdminsToBslUrl implements WebLoginListener {
  ...
  public void onLogin(IdentifiedUser user, HttpServletRequest request, HttpServletResponse response)
    ...
    if ((response.getStatus() == HttpServletResponse.SC_MOVED_TEMPORARILY
            || response.getStatus() == HttpServletResponse.SC_MOVED_PERMANENTLY)
        && (permissionBackend.user(user).testOrFalse(GlobalPermission.ADMINISTRATE_SERVER))
        && !hasValidLicence()) {
      String callbackLocation = response.getHeader("Location");
